...
bool ThreeDModel::collisionBetweenPoint(Vector3d* v, float threshold)
{
	if (theVerts == NULL)
	{
		cout << "NO VERTS" << endl;
	...
	else
	{
		for (int i = 0; i < numberOfTriangles; i++)
		{
			float dist = Vector3d::dotProduct((*v - theVerts[theFaces[i].thePoints[0]]), theFaceNormals[theFaces[i].theFaceNormal]);
...
bool ThreeDModel::BarycentricCalculation(Vector3d* point, float dist, int i)
{
	Vector3d P = *point - theFaceNormals[theFaces[i].theFaceNormal] * dist;

	// Compute vectors        
	Vector3d v0 = theVerts[theFaces[i].thePoints[2]] - theVerts[theFaces[i].thePoints[0]];
	Vector3d v1 = theVerts[theFaces[i].thePoints[1]] - theVerts[theFaces[i].thePoints[0]];
	Vector3d v2 = P - theVerts[theFaces[i].thePoints[0]];
